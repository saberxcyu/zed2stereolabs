...
This repo contains some tools we built for using the ZED2 stereo camera from Stereolabs. 
There are two main components within the repo:
- **on Windows**: 
  1. venv is managed by uv. see pyproject.toml for more details.
  2. requires the ZED SDK and CUDA (+TensorRT to speed up inference).
  3. can use the custom built software for depth sensing and pose tracking. 
...
- **on Raspberry PI**: 
  1. does not require the SDK. treats the camera like a USB web cam.
  2. scripts are inside software/tools/raspberry_pi only.
  3. comes with two scripts: record.py (for recording a video) and process.py (for processing the video recorded into left / right rectified frames)
...
## 3. Desktop pipeline -- Depth and Pose
...
### 5.3 Rectify on Pi or elsewhere
